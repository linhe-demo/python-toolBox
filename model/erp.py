...
class ErpDatatable:
    ...
    def sqlMap():
        return {
            ...
            ''',
            "update_order_info": '''
                UPDATE ecshop.ecs_order_info SET facility_id = '%s' WHERE order_id = '%s'
            ''',
            "get_purchase_info": '''
                SELECT * FROM ecshop.batch_goods_purchase_request WHERE order_id IN ('%s')
            ''',
            "update_purchase_info": '''
                UPDATE ecshop.batch_goods_purchase_request SET facility_id = '%s' WHERE order_id = '%s'
            ''',
            "get_delivery_info": '''
                SELECT * FROM eris.delivery_list_detail WHERE order_id IN ('%s')
            ''',
            "update_delivery_info": '''
                UPDATE eris.delivery_list_detail SET facility_id = '%s' WHERE order_id = '%s'
            '''
        }
